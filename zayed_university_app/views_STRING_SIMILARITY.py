...
def get_response_from_watson(request):
    ...
    df1_str = ""
    for i in top_df_extension[0]:
        df1_str += i + "\n"
    
    if len(top_df_extension[0]) > 0 and top_df_extension[1] == "link":
        return JsonResponse({'session_id': session_id_, 'answer': df1_str, 'intent': 'General', 'url': top_df_extension[0]})

    if len(top_df_extension[0]) > 0 and top_df_extension[1] == "string":
        return JsonResponse({'session_id': session_id_, 'answer': df1_str, 'intent': 'General'})
