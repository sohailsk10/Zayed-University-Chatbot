...
QUESTION_VEC = np.load('zu_data_encode_v1.npy')
print("[#] Encoding Done")
...
def get_response_from_watson(request):
    ...
    if len(res['output']['intents']) > 0 and res_conf > 0.85:
        ...
        print("_main_input_list", _main_input_list)
        listToStr = ' '.join([str(elem) for elem in _main_input_list])
        ...
        keywords = kw_model.extract_keywords(listToStr.strip().lower(), keyphrase_ngram_range=(1, 7), stop_words=None, use_mmr=True, diversity=0.7, highlight=True, top_n=10)
